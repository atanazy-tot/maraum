...
1. Create tables (profiles, scenarios, sessions, messages, logs)
2. Create indexes
3. Create triggers and functions
4. Enable Row-Level Security
5. Create RLS policies

### Seed Data Migration (002_seed_scenarios.sql)

1. Insert 3 initial scenarios with German/English messages

### Scheduled Jobs Setup

Configure one of the following:

**Option A: pg_cron (Supabase/PostgreSQL native)**
```sql
-- Run daily at 02:00 UTC
SELECT cron.schedule('cleanup-expired-sessions', '0 2 * * *', 'SELECT delete_expired_sessions()');
SELECT cron.schedule('cleanup-old-logs', '0 3 * * *', 'SELECT delete_old_logs()');
SELECT cron.schedule('reset-weekly-limits', '0 0 * * 1', 'SELECT reset_weekly_limits()');
```
...
**Option C: Application-level Cron**
- Implement in Astro backend with node-cron or similar
...
SUPABASE_SERVICE_ROLE_KEY=xxxxx  # For logging and admin operations

# Anthropic API
ANTHROPIC_API_KEY=xxxxx
ANTHROPIC_MODEL_SCENARIO=claude-4.5-haiku  # Or actual model name
ANTHROPIC_MODEL_HELPER=claude-4.5-haiku

# Prompt Template Paths (if using file-based system)
PROMPT_SCENARIO_MARKETPLACE=./prompts/scenario-marketplace.md
PROMPT_SCENARIO_PARTY=./prompts/scenario-party.md
PROMPT_SCENARIO_KEBAB=./prompts/scenario-kebab.md
PROMPT_HELPER=./prompts/helper.md

# Application Config
WEEKLY_SCENARIO_LIMIT=3
...
1. **Authentication Flow:**
   - Register new user → profile created automatically
   - Login → session cookie set
   - Logout → session invalidated

2. **Scenario Flow:**
   - Start scenario → session created, single active session enforced
   - Send messages → counts incremented via trigger
   - Complete scenario → duration calculated, completion counters incremented
   - View history → completed session appears

3. **Rate Limiting:**
   - Complete 3 scenarios → limit enforced
   - Wait for reset date → limit restored

4. **Data Cleanup:**
   - Abandoned session (7 days) → deleted by cleanup function
   - Old logs (30 days) → deleted by cleanup function

5. **Row-Level Security:**
   - User A cannot access User B's sessions/messages/profile
   - All users can read scenarios table

### No Schema Changes Required

- Additional scenarios (just INSERT into scenarios table)
- Multiple languages (add language column to scenarios, extend prompts)
- Mobile optimization (frontend only)
- Analytics dashboard (query existing data)

---

## Notes

1. **Supabase Auth Integration:** The `auth.users` table is managed by Supabase. The trigger `on_auth_user_created` automatically creates a profile when a user registers.

2. **Testing Strategy:** Use separate Supabase projects for development, staging, and production. Seed with test scenarios and users.

3. **Backup Strategy:** Supabase provides automatic backups. For self-hosted production, implement pg_dump scheduled backups.

4. **Monitoring:** Query `logs` table for error rates, API performance, and user behavior analytics. Consider exporting to external analytics platform post-MVP.

5. **Performance Optimization:** Monitor slow queries via Supabase dashboard. Add indexes as needed based on actual query patterns.

6. **Security Audit:** Review RLS policies before production launch. Test with multiple user accounts to ensure data isolation.

7. **Data Retention:** Comply with GDPR by ensuring complete account deletion removes all personal data via CASCADE constraints.

8. **Connection Pooling:** Configure Supabase connection pooling for concurrent user support. Default settings adequate for MVP; adjust based on load.
...
1. Run migration files against local Supabase instance
2. Seed test data
3. Verify triggers and functions work correctly
4. Test RLS policies with multiple test users

### Phase 2: Staging Deployment
1. Run migrations on staging Supabase project
2. Deploy application to staging server
3. Execute end-to-end testing
4. Verify cleanup functions execute correctly
5. Load test with simulated users

### Phase 3: Production Deployment
1. Run migrations on production Supabase project
2. Deploy application to production server (DigitalOcean)
3. Configure scheduled jobs for cleanup
4. Monitor logs for errors
5. Verify data isolation via RLS

### Phase 4: Monitoring
1. Weekly query performance reviews
2. Monthly storage usage checks
3. Log retention and cleanup verification
4. User growth tracking vs. database capacity
...
**2. Get completed scenarios (history):**
```sql
SELECT s.*, sc.title, sc.emoji 
FROM sessions s
...
**3. Get all messages for session replay:**
```sql
...
**5. Get session with message counts:**
```sql
SELECT id, scenario_id, message_count_main, message_count_helper, is_completed
FROM sessions 
WHERE id = $1;
```

### Analytics Queries (Post-MVP)
...
SELECT 
    ...
    COUNT(*) FILTER (
        WHERE EXISTS (
            ...
            AND started_at BETWEEN profiles.created_at + INTERVAL '1 day' AND profiles.created_at + INTERVAL '7 days'
        )
    ) AS retained_users
FROM profiles
...
**4. Average messages per scenario:**
```sql
SELECT 
    scenario_id,
    AVG(message_count_main) AS avg_main_messages,
    AVG(message_count_helper) AS avg_helper_messages
FROM sessions 
WHERE is_completed = true
GROUP BY scenario_id;
```

**5. API error rate (from logs):**
```sql
SELECT 
    DATE(created_at) AS log_date,
    COUNT(*) FILTER (WHERE event_type LIKE 'api_%' AND level = 'error') AS errors,
    COUNT(*) FILTER (WHERE event_type LIKE 'api_%') AS total_calls,
    ROUND(COUNT(*) FILTER (WHERE event_type LIKE 'api_%' AND level = 'error') * 100.0 / NULLIF(COUNT(*) FILTER (WHERE event_type LIKE 'api_%'), 0), 2) AS error_rate
FROM logs
WHERE event_type LIKE 'api_%'
GROUP BY DATE(created_at)
